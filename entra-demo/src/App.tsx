...
export default function App() {
  ...
  async function probarToken() {
    ...
    setHasToken(true); // Se marca que el token fue obtenido con éxito
    setSalida(
  ...
  async function consultar() {
    if (account) {
      const resultado = await consultarApi(instance, account);
      // Si la respuesta es un objeto JSON, lo formateamos para visualización limpia
      setSalida(
  ...
  return (
    <main style={{ maxWidth: 850, margin: '40px auto', padding: 20 }}>
      ...
      <pre style={{ whiteSpace: 'pre-wrap', overflowWrap: 'anywhere', marginTop: 20 }}>
        {salida}
      </pre>
    </main>
  );
